...
# 8. Grain Serialization

   The `[GenerateSerializer]` attribute tells Orleans to automatically generate serialization logic for the class.
   Orleans requires classes that are passed between grains or persisted to have serialization, 
   and this attribute simplifies the process by **generating that code at compile time**.

   The `[Id(x)]` attribute is used to identify fields that should be serialized and assigns them a unique ID.
   Orleans requires that each serializable field is tagged with a unique identifier for versioning and efficient 
   serialization. These IDs are used internally during serialization/deserialization and help Orleans keep track 
   of each field.

   **Why are these attributes important?**
   - **Performance**: They enable Orleans to generate highly efficient serialization logic specific to the class.
   - **Versioning**: Using `Id(x)` allows you to evolve the class without breaking backward compatibility with previously serialized data.
   - **Simplicity**: By using `[GenerateSerializer]`, you avoid having to manually write complex serialization code.
   ...
   > The serialization code generated by the `[GenerateSerializer]` attribute in Orleans is typically done at **compile time**.
   > The generated code is not stored in a separate file that you can see in your project folder.
   > After building your project, you can open the output assembly with a tool like **ILSpy**.

# 9. Streams

1. Add the library **Microsoft.Orleans.Streamsing** to the project.

   ```xml
   <PackageReference Include="Microsoft.Orleans.Streaming" Version="8.2.0" />
   ```

2. Update the `Program.cs` file to inject the **Streams** services:
   ...
   siloBuilder.AddMemoryStreams(ORLEANS_STREAM_PROVIDER);
...
3. Add a new class `StreamItem` to store the message content:
...
4. Update the `UrlShortenerGrain.SetUrl` method to send a message to the stream:
   ```csharp
   await _urlDetails.WriteStateAsync();
...
5. Update the user interface to subscribe to this stream and display the URL created.

   ```html
   /* Home.razor /*

   <div>
      <FluentButton OnClick="@Button_Click" Appearance="Appearance.Accent" Style="margin: 24px 0px;">
          Send
      </FluentButton>
      <FluentListbox Items="@Messages" />
   </div>
   ```
